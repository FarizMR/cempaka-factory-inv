            ...
            <div class="card-body">
                ...
                <div v-else>
                    <form @submit.prevent="submitForm">
                        ...
                        <div class="form-group row">
                            <label class="col-form-label col-sm-2" for="pemasok"
                                >Pemasok:</label
                            >
                            <div class="col-sm-10">
                                <select
                                    id="pemasok"
                                    v-model="selectedPemasok"
                                    class="form-control form-control-sm"
                                >
                                    <option
                                        v-for="pemasok in pemasokOptions"
                                        :value="pemasok.id"
                                        :key="pemasok.id"
                                    >
                                        {{ pemasok.nama }}
...
export default {
    ...
    methods: {
        async barang_transaksi() {
            try {
                await axios
                    .get(`/api/barang-masuk/${this.id}`)
                    .then(({ data }) => {
                        this.barangMasuks = data;

                        this.id = this.barangMasuks.id;
                        this.tanggal = this.barangMasuks.tanggal;
                        this.selectedPemasok = this.barangMasuks.pemasok_id;
                        this.selectedBarang = this.barangMasuks.jenis_barang_id;

                        this.barangMasuks.jumlah_barang_masuk.forEach(
                            (item) => {
        ...
        async submitForm() {
            try {
                const response = await axios.put(`/api/barang-masuk/${this.id}`, {
                    tanggal: this.tanggal,
                    pemasok_id: this.selectedPemasok,
                    barang_id: this.selectedBarang,
                    jumlah_barang_masuk: this.satuanItems.map((item) => {
                        return {
